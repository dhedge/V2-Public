...
contract AaveV3sUSDeJUL25TestEthereum is AaveV3TestSetup, EthereumSetup {
  ...
  function _createAaveV3TestConfig() internal pure returns (AaveV3TestSetup.AaveV3TestConfig memory config) {
    ...
    config.token0ToLend = EthereumConfig.PT_sUSDe_JUL_2025;
    config.token1ToLend = EthereumConfig.USDC;
    ...
    config.token0Oracle = EthereumConfig.PT_sUSDe_JUL_2025_PRICE_AGGREGATOR;
    config.token1Oracle = EthereumConfig.USDC_CHAINLINK_ORACLE;
    ...
    config.token0ToLendPendleMarket = EthereumConfig.PENDLE_MARKET_sUSDe_JUL_2025;
    config.token0ToLendUnderlying = EthereumConfig.sUSDe;
    config.token0ToLendUnderlyingOracle = EthereumConfig.sUSDe_CHAINLINK_ORACLE;
    config.useEMode = 8; // PT sUSDe Stablecoins JUL 2025 eMode ID
  }
