...
describe("Balancer V2 Test", function () {
  ...
  it("should be able to swap tokens on balancer - swap exactInput.", async () => {
    ...
    await expect(poolLogicProxy.connect(manager).execTransaction(balancer.v2Vault, swapTx)).to.be.revertedWith(
      "sender or recipient is not pool",
    );
    swapTx = iBalancerV2Vault.encodeFunctionData("swap", [
      [poolId, kind, assetIn, assetOut, amount, "0x"],
    ...
    swapTx = iBalancerV2Vault.encodeFunctionData("swap", [
      [poolId, kind, assetIn, assetOut, amount, "0x"],
      [sender, false, recipient, false],
      "949700", // Not putting 950000 because of rounding errors. Expect 5% slippage impact.
      Math.floor(Date.now() / 1000 + 100000000),
    ]);
    await expect(poolLogicProxy.connect(manager).execTransaction(balancer.v2Vault, swapTx)).to.be.revertedWith(
  ...
  it("should be able to swap tokens on balancer - swap exactOutput.", async () => {
    ...
    swapTx = iBalancerV2Vault.encodeFunctionData("swap", [
      [poolId, kind, assetIn, assetOut, amount, "0x"],
      [sender, false, recipient, false],
      "1053000", // Creating a scenario for 5% slippage.
      Math.floor(Date.now() / 1000 + 100000000),
    ]);
    await expect(poolLogicProxy.connect(manager).execTransaction(balancer.v2Vault, swapTx)).to.be.revertedWith(
      "slippage impact exceeded",
  ...
  it("should revert in case of high cumulative slippage impact", async () => {
    ...
    // Because of the previous two transaction, cumulative slippage should be around 4%.
    // The following transaction should revert as cumulative slippage will be around 6%.
    await expect(poolLogicProxy.connect(manager).execTransaction(balancer.v2Vault, swapTx)).to.be.revertedWith(
      "slippage impact exceeded",
    );
  });
  ...
  it("should revert if caller is not the manager but affects the pool of some manager", async () => {
    ...
    await evilPoolManager.givenCalldataReturnBool(
      iHasSupportedAsset.encodeFunctionData("isSupportedAsset", [assetOut]),
      true,
    );

    await expect(
      deployments.balancerV2Guard?.connect(anon).txGuard(evilPoolManager.address, balancer.v2Vault, swapTx),
    ).to.be.revertedWith("Caller not authorised");

    await expect(
      deployments.balancerV2Guard?.connect(anon).txGuard(poolManagerLogicProxy.address, balancer.v2Vault, swapTx),
    ).to.be.revertedWith("Caller not authorised");

    expect(
      (await deployments.slippageAccumulator.managerData(poolManagerLogicProxy.address)).accumulatedSlippage,
    ).to.equal(ethers.constants.Zero, "Slippage impact detected");
  });
});
