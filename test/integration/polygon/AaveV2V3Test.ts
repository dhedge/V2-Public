...
import { aaveV2, assets, assetsBalanceOfSlot, aaveV3 } from "../../../config/chainData/polygon-data";
import {
  IERC20,
  IERC20__factory,
...
import { IAaveV3Pool__factory } from "../../../types/factories/IAaveV3Pool__factory";
...
describe("Aave V2 & V3 Test", function () {
  ...
  it("Should be able to deposit usdc: both V2, V3", async () => {
    ...
    const V2AMUSDC = await ethers.getContractAt("IERC20", aaveV2.aTokens.usdc);
    checkAlmostSame(await V2AMUSDC.balanceOf(poolLogicProxy.address), amount);
    const V3AMUSDC = await ethers.getContractAt("IERC20", aaveV3.aTokens.usdc);
  ...
  it("Should be able to borrow dai/usdc: both V2, V3", async () => {
    ...
    const V2VariableDebtDai = await ethers.getContractAt("IERC20", aaveV2.variableDebtTokens.dai);
    checkAlmostSame(await V2VariableDebtDai.balanceOf(poolLogicProxy.address), amount.div(2));
    const V3VariableDebtUsdc = await ethers.getContractAt("IERC20", aaveV3.variableDebtTokens.usdc);
