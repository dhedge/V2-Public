...
const { sushi, aave, assets, price_feeds } = require("../../../config/chainData/ethereum-data");
...
describe("Aave Test", function () {
  ...
  describe("Aave", () => {
    ...
    it("Should be able to deposit weth and receive aweth", async () => {
      // Pool balance: 50 USDC, 50 USDT, $50 in WETH
      // Aave balance: 100 aUSDC, 50 debtUSDT

      const amount = await WETH.balanceOf(poolLogicProxy.address);
      ...
      expect(wethBalanceBefore).to.be.equal(amount);
      ...
      checkAlmostSame(awethBalanceAfter, amount);
      checkAlmostSame(await poolManagerLogicProxy.totalFundValue(), totalFundValueBefore);
    });

    // it("Should be able to borrow USDT", async () => {
    //   // Pool balance: 50 USDC
    //   // Aave balance: 100 aUSDC, $50 in aWETH
    ...
    it("Should be able to borrow WETH", async () => {
      // Pool balance: 50 USDC, 50 USDT
      // Aave balance: 100 aUSDC, 50 debtUSDT, $50 in aWETH

      const amount = (1e16).toString();
    ...
    it("should be able to withdraw", async function () {
      ...
      const wethBalanceBefore = ethers.BigNumber.from(await WETH.balanceOf(logicOwner.address));
      ...
      const wethBalanceAfter = ethers.BigNumber.from(await WETH.balanceOf(logicOwner.address));
