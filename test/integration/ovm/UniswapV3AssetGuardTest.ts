...
describe("UniswapV3AssetGuardTest", function () {
  ...
  before(async function () {
    ...
    await getAccountToken(units(18000, 6), logicOwner.address, assets.usdt, assetsBalanceOfSlot.usdt);
    ...
    await deployments.assets.USDT.approve(poolLogicProxy.address, units(6000, 6));
    await poolLogicProxy.deposit(assets.usdt, units(6000, 6));
  ...
  describe("Unsuppored Assets", () => {
    ...
    it("tokenPrice and withdraw works if both LP assets are unsupported", async () => {
      // Mint Uniswap v3 LP
      await assetHandler.removeAsset(assets.wbtc); // Remove LP assets first
      await assetHandler.removeAsset(assets.dai);
      ...
      const mintSettings: UniV3LpMintSettings = {
        ...
        token1,
        fee,
        amount0: units(1).div(100),
        amount1: units(400),
        tickLower: tick - tickSpacing,
        tickUpper: tick + tickSpacing,
      };
      await mintLpAsUser(nonfungiblePositionManager, user, mintSettings, [0, 2]);
