...
import { aave, assets, assetsBalanceOfSlot, zipswap } from "../../../config/chainData/ovm-data";
import {
  IERC20,
  IERC20__factory,
...
describe("Aave Edge Test", function () {
  ...
  it("Should be able to repay DAI", async () => {
    ...
    // First approve USDC
    let approveABI = iERC20.encodeFunctionData("approve", [zipswap.router, sourceAmount]);
    await poolLogicProxy.connect(manager).execTransaction(assets.usdc, approveABI);

    const swapABI = iSushiswapV2Router.encodeFunctionData("swapExactTokensForTokens", [
      sourceAmount,
      await getAmountOut(zipswap.router, sourceAmount, [assets.usdc, assets.weth, assets.dai]),
      [assets.usdc, assets.weth, assets.dai],
      poolLogicProxy.address,
      Math.floor(Date.now() / 1000 + 100000000),
    ]);
    await poolLogicProxy.connect(manager).execTransaction(zipswap.router, swapABI);
