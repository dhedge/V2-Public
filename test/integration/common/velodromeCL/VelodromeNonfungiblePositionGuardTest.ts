...
import { getCurrentTick, mintLpAsPool, mintLpAsUser, VelodromeCLMintSettings } from "../../utils/velodromeCLUtils";
import { utils } from "../../utils/utils";
import { IBackboneDeployments, deployBackboneContracts } from "../../utils/deployContracts/deployBackboneContracts";
import { IVelodromeCLTestParams, deployVelodromeCLInfrastructure } from "./velodromeCLTestDeploymentHelpers";

const deadLine = Math.floor(Date.now() / 1000 + 100000000);

export const velodromeCLNonfungiblePositionGuardTest = (testParams: IVelodromeCLTestParams) => {
  const { pairs, factory } = testParams;
  const { bothSupportedPair, token0UnsupportedPair, token1UnsupportedPair } = pairs;

  describe("Velodrome CL Test", function () {
    let deployments: IBackboneDeployments;
    let logicOwner: SignerWithAddress, manager: SignerWithAddress;
    let poolFactory: PoolFactory, poolLogicProxy: PoolLogic, poolManagerLogicProxy: PoolManagerLogic;
    let tokenId: BigNumber;
    let velodromeNonfungiblePositionGuard: VelodromeNonfungiblePositionGuard;
    ...
    before(async function () {
      ...
      ({ nonfungiblePositionManager, testSupportedAsset, velodromeNonfungiblePositionGuard } =
        await deployVelodromeCLInfrastructure(deployments, testParams));
    });
    ...
    it("Can't mint position if nft position is not enabled", async () => {
      ...
      ).to.revertedWith("velodrome cl asset not enabled");
    });
    ...
    it("Can't mint position with unsupported assets", async () => {
      ...
      mintSettings.tickSpacing = token0UnsupportedPair.tickSpacing;
      ...
      mintSettings.tickSpacing = token1UnsupportedPair.tickSpacing;
    ...
    it("Mint a position & Check nft-tracker", async () => {
      ...
      expect(await velodromeNonfungiblePositionGuard.getOwnedTokenIds(poolLogicProxy.address)).to.deep.equal([]);

      await mintLpAsPool(nonfungiblePositionManager.address, poolLogicProxy, manager, mintSettings);
      const tokenId = await nonfungiblePositionManager.tokenOfOwnerByIndex(poolLogicProxy.address, 0);

      // 1 tracked nft after mint
      expect(await velodromeNonfungiblePositionGuard.getOwnedTokenIds(poolLogicProxy.address)).to.deep.equal([tokenId]);
    });
