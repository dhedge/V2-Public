...
import { deployContracts, IDeployments, NETWORK, IAssetSetting } from "../utils/deployContracts";
...
export const uniswapV3NonfungiblePositionGuardTest = (params: IUniswapV3NonfungiblePositionGuardTestParameter) => {
  ...
  describe("Uniswap V3 LP Test", function () {
    ...
    beforeEach(async function () {
      ...
      approveABI = iERC20.encodeFunctionData("approve", [uniswapV3.router, bothSupportedPair.amount0.mul(3)]);
      await poolLogicProxy.connect(manager).execTransaction(bothSupportedPair.token0, approveABI);
      approveABI = iERC20.encodeFunctionData("approve", [uniswapV3.router, bothSupportedPair.amount1.mul(3)]);
      await poolLogicProxy.connect(manager).execTransaction(bothSupportedPair.token1, approveABI);
    ...
    describe("After position", () => {
      ...
      it("Check withdraw after decreasing liquidity to zero (no collect)", async () => {
        ...
        expect(
          await (await ethers.getContractAt("IERC20", bothSupportedPair.token0)).balanceOf(logicOwner.address),
        ).closeTo(usdcBalanceBefore.add(bothSupportedPair.amount0.mul(3)), 1);
        expect(
          await (await ethers.getContractAt("IERC20", bothSupportedPair.token1)).balanceOf(logicOwner.address),
        ).closeTo(wethBalanceBefore.add(bothSupportedPair.amount1.mul(3)), 1);
      });
      ...
      it("Should be able to withdraw", async () => {
        ...
        expect(await (await ethers.getContractAt("IERC20", bothSupportedPair.token0)).balanceOf(logicOwner.address)).gt(
          usdcBalanceBefore,
        );
        expect(await (await ethers.getContractAt("IERC20", bothSupportedPair.token1)).balanceOf(logicOwner.address)).gt(
          wethBalanceBefore,
        );
