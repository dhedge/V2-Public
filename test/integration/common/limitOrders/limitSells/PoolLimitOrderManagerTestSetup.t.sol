...
import {EasySwapperV2Mock, IEasySwapperV2Mock} from "test/integration/common/limitOrders/IEasySwapperV2Mock.sol";
import {IPoolFactoryMock, IHasSupportedAssetMock} from "test/integration/common/limitOrders/IPoolFactoryMock.sol";
import {IAssetHandlerMock} from "test/integration/common/limitOrders/IAssetHandlerMock.sol";
...
abstract contract PoolLimitOrderManagerTestSetup is Test {
  ...
  function test_revert_create_limit_order_if_not_pool() public {
    vm.startPrank(user);
    vm.expectRevert(abi.encodeWithSelector(ICommonErrors.InvalidPool.selector, swapper));
    poolLimitOrderManagerProxy.createLimitOrder(
      PoolLimitOrderManager.LimitOrderInfo({
        amount: 1000e18,
        stopLossPriceD18: 0,
