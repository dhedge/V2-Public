...
export const uniswapV3SwapRouterGuardTest = (params: IUniswapV3SwapRouterGuardTestParameter) => {
  ...
  describe("Uniswap V3 Swap Router Test", function () {
    ...
    beforeEach(async function () {
      ...
      await (await ethers.getContractAt("IERC20", pair.token0)).approve(poolLogicProxy.address, pair.amount0.mul(3));
      await poolLogicProxy.deposit(pair.token0, pair.amount0.mul(3));
      await (await ethers.getContractAt("IERC20", pair.token1)).approve(poolLogicProxy.address, pair.amount1.mul(3));
