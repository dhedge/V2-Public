...
abstract contract OdosContractGuardTestSetup is BackboneSetup, OdosAPIHelper {
  address private immutable odosRouter;

  PoolLogic private testPool;
  PoolManagerLogic internal fundManagerLogic;
  OdosV2ContractGuard private odosContractGuard;
  uint256 public chainId;
  string public odosVersion;

  constructor(address _odosRouter, uint256 _chainId, string memory _odosVersion) {
    odosRouter = _odosRouter;
    chainId = _chainId;
    odosVersion = _odosVersion;
  }
  ...
  function setUp() public virtual override {
    ...
    if (keccak256(abi.encodePacked((odosVersion))) == keccak256(abi.encodePacked(("v2")))) {
      odosContractGuard = new OdosV2ContractGuard(address(slippageAccumulator));
    } else if (keccak256(abi.encodePacked((odosVersion))) == keccak256(abi.encodePacked(("v3")))) {
      odosContractGuard = OdosV2ContractGuard(address(new OdosV3ContractGuard(address(slippageAccumulator))));
    } else {
      revert("Unknown Odos version");
    }
    // Set the odos contract guard in the governance contract.
    governance.setContractGuard({extContract: odosRouter, guardAddress: address(odosContractGuard)});
    ...
    bytes memory approveCallData = abi.encodeWithSelector(IERC20.approve.selector, odosRouter, type(uint256).max);
  ...
  function _getSwapData(
    ...
    (destAmount_, calldata_) = getDataFromOdos(params, chainId, isCompact, odosVersion);
  }
  ...
  function _runTxGuardTest(bool isCompact, bool isAfterGuard) internal {
    ...
    if (isAfterGuard) {
      odosContractGuard.afterTxGuard(address(fundManagerLogic), odosRouter, swapdata);
    } else {
      odosContractGuard.txGuard(address(fundManagerLogic), odosRouter, swapdata);
    }
  }
  ...
  function _runRecipientTest(bool isCompact) internal {
    ...
    testPool.execTransaction(odosRouter, swapdata);
  }
  ...
  function _wipeInputAmountInMemory(bytes memory data) internal pure returns (bytes memory result) {
    assembly {
  ...
  function _runZeroAmountTest(bool isCompact) internal {
    ...
    bytes memory mData;
    if (!isCompact) {
      bytes memory params = odosContractGuard.getParams(swapdata);
      ...
      mData = abi.encodePacked(odosContractGuard.getMethod(swapdata), encodedData);
    } else {
      mData = _wipeInputAmountInMemory(swapdata);
    }

    vm.expectRevert("invalid input amount");
    testPool.execTransaction(odosRouter, mData);
  }
