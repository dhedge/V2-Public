...
describe("PoolFactory", function() {
    before(async function(){
        ...
        await poolFactory.connect(dao).setGuard(uniswapV2Router, uniswapV2Guard.address);
    });
    ...
    it("Should be able to get WETH", async function() {
        ...
        await UniswapRouter.swapExactTokensForTokens(5e18.toString(), 0, [weth, usdc], logicOwner.address, Math.floor(Date.now() / 1000 + 10000));
    });
    ...
    it("should be able to swap tokens on uniswap.", async () => {
        ...
        swapABI = iUniswapV2Router.encodeFunctionData("swapExactTokensForTokens", [sourceAmount, 0, [usdc, weth], poolLogicProxy.address, Math.floor(Date.now() / 1000 + 10000)]);
