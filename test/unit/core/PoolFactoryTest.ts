...
describe("PoolFactory", function () {
  ...
  before(async function () {
    ...
    const Governance = await ethers.getContractFactory("GovernanceV24");
    ...
    await poolFactory.setMaximumFee(5000, 300, 100, 100); // 3% streaming fee, 1% entry fee, 1% exit fee.
    ...
    uniswapV2RouterGuard = <UniswapV2RouterGuard>await UniswapV2RouterGuard.deploy();
    ...
    const oneInchV4Guard = await OneInchV4Guard.deploy();
  ...
  it("should be able to manage assets", async function () {
    ...
    ).to.be.revertedWith("only manager, owner or trader enabled");
  ...
  it("should be able to withdraw", async function () {
    ...
    await poolManagerLogicProxy.connect(manager).announceFeeIncrease(5000, 200, 0, 0); // increase streaming fee to 2%
  ...
  it("should be able to manage fees", async function () {
    ...
    await expect(poolManagerLogicManagerProxy.announceFeeIncrease(6100, 400, 25, 25)).to.be.revertedWith(
      "exceeded allowed increase",
    );
    await expect(poolManagerLogicManagerProxy.announceFeeIncrease(4000, 400, 25, 25)).to.be.revertedWith(
      "exceeded allowed increase",
    );

    //Can set manager fee
    await poolManagerLogicManagerProxy.announceFeeIncrease(4000, 250, 25, 25); // increase streaming fee to 2.5% , entry and exit fees to 0.25%
    ...
    let [performanceFeeNumerator, managerFeeNumerator, entryFeeNumerator, exitFeeNumerator, managerFeeDenominator] =
    ...
    await expect(poolManagerLogicManagerProxy.setFeeNumerator(3000, 300, 25, 25)).to.be.revertedWith(
      "manager fee too high",
    );
    await expect(poolManagerLogicManagerProxy.setFeeNumerator(3000, 300, 100, 25)).to.be.revertedWith(
      "manager fee too high",
    );
    await expect(poolManagerLogicManagerProxy.setFeeNumerator(3000, 300, 25, 100)).to.be.revertedWith(
      "manager fee too high",
    );

    await poolManagerLogicManagerProxy.setFeeNumerator(3000, 200, 0, 0);
    [performanceFeeNumerator, managerFeeNumerator, entryFeeNumerator, exitFeeNumerator, managerFeeDenominator] =
      await poolManagerLogicManagerProxy.getFee();
    ...
    expect(exitFeeNumerator.toString()).to.equal("0");
  });
