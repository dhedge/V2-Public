...
describe("RewardDistribution Tests", () => {
  ...
  const prepareMockedWhitelistedPools = async (
    ...
    await mockedPoolFactory.mock.getAssetPrice.returns(rewardTokenPrice);
    return Promise.all(
      Array.from(Array(pools.length).keys(), async (element) => {
        const mockedPoolManagerLogic = await waffle.deployMockContract(owner, PoolManagerLogic__factory.abi);
        await mockedPoolManagerLogic.mock.totalFundValue.returns(poolValue);
        await mockedPoolManagerLogic.mock.isSupportedAsset.returns(pools[element]);
        const mockedPoolLogic = await waffle.deployMockContract(owner, PoolLogic__factory.abi);
        await mockedPoolLogic.mock.poolManagerLogic.returns(mockedPoolManagerLogic.address);
        await mockedPoolLogic.mock.factory.returns(mockedPoolFactory.address);
