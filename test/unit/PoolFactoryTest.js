...
describe('PoolFactory', function () {
  ...
  describe('AssetHandler', function () {
    it('only owner should be able to remove assets', async function () {
      ...
      await expect(assetHandler.connect(manager).removeAsset(slink)).to.be.revertedWith('Ownable: caller is not the owner');
    ...
    it('only owner should be able to add asset/assets', async function () {
      await expect(assetHandler.connect(manager).addAsset(slink, 0, link_price_feed.address)).to.be.revertedWith('Ownable: caller is not the owner');
      await expect(assetHandler.connect(manager).addAssets([{ asset: slink, assetType: 0, aggregator: link_price_feed.address }])).to.be.revertedWith('Ownable: caller is not the owner');
    ...
    it('only owner should be able to set chainlink timeout', async function () {
      ...
      await expect(assetHandler.connect(manager).setChainlinkTimeout(90)).to.be.revertedWith('Ownable: caller is not the owner');
    ...
    it('only owner should be able to enable/disable chainlink', async function () {
      expect(await assetHandler.isDisabledChainlink()).to.be.false;

      await expect(assetHandler.connect(manager).disableChainlink()).to.be.revertedWith('Ownable: caller is not the owner');
      await expect(assetHandler.connect(manager).enableChainlink()).to.be.revertedWith('Ownable: caller is not the owner');

      await assetHandler.disableChainlink();

      expect(await assetHandler.isDisabledChainlink()).to.be.true;

      await assetHandler.enableChainlink();
    ...
    it('only owner should be able to set poolFactory', async function () {
      ...
      await expect(assetHandler.connect(manager).setPoolFactory(ZERO_ADDRESS)).to.be.revertedWith('Ownable: caller is not the owner');

      await assetHandler.setPoolFactory(ZERO_ADDRESS);

      expect(await assetHandler.poolFactory()).to.be.equal(ZERO_ADDRESS);
