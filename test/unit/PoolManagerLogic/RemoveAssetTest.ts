...
describe("RemoveAssets", function () {
  ...
  before(async function () {
    ...
    const ADDRESSES_PROVIDER_ABI = iAaveProtocolDataProvider.encodeFunctionData("ADDRESSES_PROVIDER", []);
    await aaveProtocolDataProvider.givenCalldataReturnAddress(ADDRESSES_PROVIDER_ABI, aaveProtocolDataProvider.address);

    // mock ILendingPoolAddressesProvider
    const ILendingPoolAddressesProvider = await artifacts.readArtifact("ILendingPoolAddressesProvider");
    ...
    await governance.setAddresses([
      ...
      { name: toBytes32("aaveLendingPoolV2"), destination: aaveLendingPool.address },
    ]);
