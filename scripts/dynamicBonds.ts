...
const deployDynamicBonds = async (hre: HardhatRuntimeEnvironment) => {
  ...
  const dynamicBondsImplementationAddress = ethers.utils.hexValue(
    await provider.getStorageAt(dynamicBondsProxy.address, implementationStorage),
  );
...
const getDynamicBondsContract = async (hre: HardhatRuntimeEnvironment, version: any): DynamicBonds => {
  const ethers = hre.ethers;
