...
import { IDeployedOracle, IJob, IProposeTxProperties, IUpgradeConfig, IVersions, TDeployedAsset } from "../../types";
...
export const assetsJob: IJob<void> = async (
  ...
  const newAssets: TDeployedAsset[] = versions[config.newTag].contracts.Assets.map((versionAsset) => {
    const jsonAsset = jsonAssets.find((x) => x.assetAddress === versionAsset.assetAddress);
    if (!jsonAsset) {
      throw new Error("Missing asset in jsonAssets" + versionAsset.assetAddress);
    }
    return {
      ...jsonAsset,
      oracleAddress: versionAsset.oracleAddress,
    };
  });
  ...
  versions[config.newTag].contracts.Assets = [...newAssets];
};
